...
import soundForget from './assets/sound-forget.mp3'
import { randomString, waitTimeout, getUrlParameters, getRandomElements } from './utils'
import { EasySvg, MediumSvg, HardSvg } from './components/tagSvg'
...
const makeScene: (level: number, icons: Icon[]) => Scene = (level, icons) => {
  ...
  while (compareLevel > 0) {
    iconPool.push(...iconPool.slice(0, Math.min(0, 2 * (compareLevel - 5))))
    compareLevel -= 5
  }
...
const Symbol: FC<SymbolProps> = ({ x, y, icon, isCover, status, onClick }) => {
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className="symbol-inner" style={{ opacity: isCover ? 0.5 : 1 }}>
        {typeof icon.content === 'string' ? <i>{icon.content}</i> : icon.content}
      </div>
    </div>
  )
}
...
const App: FC = () => {
  ...
  useEffect(() => {
    ...
    if (paramsProgress > 0 || paramsLevel > 1) {
      if (
        paramsProgress !== Math.min(3, Math.floor((progress / totalIcons) * 3)) &&
        paramsLevel != level
      ) {
        restart(paramsLevel, paramsProgress / 3)
      } else if (paramsLevel != level) {
        restart(paramsLevel)
      } else if (paramsProgress !== Math.min(3, Math.floor((progress / totalIcons) * 3))) {
  ...
  const levelUp = () => {
    ...
    if (level > maxLevel) {
      ...
      setReviseCount((reviseCountLevel[String(level + 1)] as number) || 5)
    }
    // set refresh count
    if (level > maxLevel) {
      setRefreshCount(5)
    } else {
      setRefreshCount((reviseCountLevel[String(level + 1)] as number) || 5)
    }
    // clear time handle
  ...
  const clickSymbol = async (idx: number) => {
    ...
    if (updateQueue.length === 7) {
      setTipText('Wrong Answer! Seems like you need more practice to solve it')
      setFinished(true)
    }
    ...
    <>
      <div className="leetcode-viewport">
        ...
        <CodeEditor
          defaultCode={
            ...
            levelTitleForLevel?.code.length > Math.min(3, Math.floor((progress / totalIcons) * 3))
              ? levelTitleForLevel?.code[Math.min(3, Math.floor((progress / totalIcons) * 3))]
              : ''
          }
        />
      </div>
      <div className="game-viewport">
        ...
        <div className="flex-container flex-between">
          ...
          <button className="flex-grow" onClick={undo} disabled={reviseCount === 0 || toNextLevel}>
            Revise({reviseCount})
          </button>
          {/* <button className="flex-grow" onClick={pop}>
                  Popup
              </button> */}
          <button className="flex-grow" onClick={wash} disabled={refreshCount === 0 || toNextLevel}>
            Refresh({refreshCount})
          </button>
          <button className="flex-grow colorful-bg" hidden={!toNextLevel} onClick={levelUp}>
            Next
          </button>
        </div>
        ...
        {tipText && tipText.length > 0 && (
          <div className="modal">
            <h1>{tipText}</h1>
            <div className="flex-container flex-between">
              <button className="flex-grow" onClick={() => restart(1, 0)}>
                Quit
              </button>
              ...
              {level === maxLevel && (
                <button className="flex-grow colorful-bg" onClick={enterEndlessMode}>
                  Endless Mode
                </button>
              )}
            </div>
        ...
        <button className="bgm-button" onClick={() => setBgmOn(!bgmOn)}>
          {bgmOn ? '🎶' : '🔇'}
          <audio ref={bgmRef} loop src={soundForget} />
        </button>
        <div className="bgm-gif-left" hidden={!bgmOn}>
          <img className="bgm-gif-img" src={doge} />
        </div>
